...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('cook') then
            ...
            if WarMenu.Button('Cook Meat') then
                       TriggerServerEvent("def_cookfood:getmeat")   
            end    
            if WarMenu.Button('Cook fish') then
...
local _source = source
		if campfire ~= 0 then
			...
			exports['progressBars']:startUI(10000, "يتم تنصيب النار عشان الطهي...")
			Citizen.Wait(10000)
...
Citizen.CreateThread(function()
	...
	local playerPed = PlayerPedId()
        ...
        if whenKeyJustPressed(keys['']) then
			exports['progressBars']:startUI(10000, "ازالة النار...")
			Citizen.Wait(10000)
            if campfire ~= 0 then
            SetEntityAsMissionEntity(campfire)
            DeleteObject(campfire)
